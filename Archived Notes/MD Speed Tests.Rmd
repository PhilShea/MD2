...
These test were run on the following Dell XPS 8920 PC purchased from Microenter on 31 October 2017:

```
Device name	FRPC
Processor	Intel(R) Core(TM) i7-7700 CPU @ 3.60GHz   3.60 GHz
Installed RAM	16.0 GB
Device ID	558DC6C1-F078-4397-A875-9351391F05F0
Product ID	00325-96154-34956-AAOEM
...
The Julia modules will change as I go through this process, but the test source will be pasted here.  
...
Just for completeness, dropped backed to declaring `ds`, still using `BLAS`.  No significant change.
...
Here are the main changes. In `MDRecord2`, the arrays `p` & `v` are now type `Vector{ SVector{2, T}}`. The main part of `CorrShell!` is only slightly different:
...
Clearly too many allocations going on, so trying to reduce them.  The first try, by modifying `gfij` from returning `[pe, [fx, fy]]` to simply `[pe, fx, fy]` yielded a factor of two:
...
9 runs, 20 repetitions each for 180 5,000 step runs (forgot to start Julia with `-t Auto`:

```
402.851736 seconds (10.36 G allocations: 196.385 GiB, 6.57% gc time, 2.21% compilation time: 9% of which was recompilation)
```

That's 0.45s per 1k step run.

## Longest Run

```
list1 = ["0027", "0029", "0030", "0031", "0032", "0033", "0038"]
td = "H:\\Data\\DSP\\MD\\Data"
sname = "S2b."
start = 'V'

@time MD2.filerun_thread( td, list1, sname, start, 20000, 10)

151.941826 seconds (16.10 G allocations: 303.542 GiB, 14.68% gc time, 19 lock conflicts, 0.14% compilation time)
```
That's seven files, 20K steps, 10 times each, or 1,400,000 steps in 152 s. 0.109 s per 1k steps.