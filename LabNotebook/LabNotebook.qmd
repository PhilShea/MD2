...
![Summary of all S1 data.](S1AllSummary.PNG)
...
\[![Kinetic Energy vs. mean Total Energy for series 1.](S1kevsTE.PNG)\]
...
![Zoomed in summary of S1 data.](S1Summary.PNG)
...
![Summary plot of initial S1 run.](S2%20Initial%20Result.PNG)
...
![Energy loss (black) and standard deviation (red) as a percent of the total energy. The energy loss exceeds the standard deviation of the energy by a significant amount.](S2%20Energy%20Loss.PNG)
...
![Combined runs of files S2.0036. The drop in total energy (`TE`) can be seen immediately after the start. Incidentally, the run S2.00036 had the worst correlation.](Combined%20runs%20S2%2036.PNG)

The division between potential and kinetic energy was also examined. The plot below shows this.

![Potential energy as a fraction of the kinetic energy, plotted against the mean kinetic energy. The transition region is quite sharp.](PE%20vs%20KE.PNG)
...
![Full spectrum of run S2.0036 less the first 2k points.](x36spec.png)

Zooming in we can see the following:

![The blue lines contain 99% of the signals power.](x36%20spec%20zoom.png)
...
![Final summary of the S2 initial runs. Note that this summary has the Nelson-Halperin order parameter $|\psi|$ plotted.](S2%20last%20summ.PNG)
...
The resampled data with a regression on the last 2/3 of the data is represented in @fig-x33resamp below:
...
![Absolute value of the estimate vs. the standard error. Black points are failures, green are full length passes, and blue are shortened passes. The red line shows equality (estimate equals the standard error).](findlmlentst.PNG)
...
![Run S2B.0027 after one million steps. Melting is most obvious in the imaginary component of the Nelson-Halperin order parameter (\`Impsi\`) after time $6,000\tau$ (600k steps).](Mainscalars%20x271M.png){width="600"}

We can see that the total energy is decreasing, which might be concerning. The reduction is `{r} 100 * (max(xn27$TE) - min(xn27$TE))/mean(xn27$TE)` (0.8%). This is likely due to particles slipping out of the `rskn` range of the potential. Note that simultaneously, the potential energy (`pe`) is increasing and the kinetic energy (`ke`) is decreasing.
